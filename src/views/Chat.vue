  ...
  <div class="chat">
    <v-container v-if="loggedIn == false" class="white--text container" d-flex align-center>
      <v-layout v-if="account == false">
        <v-flex>
          <v-text-field v-model="email" single-line dark label="Email address" type="email"></v-text-field>
          <v-text-field
            v-model="password"
            single-line
            dark
          ...
          <h5>
            No account?
            <v-btn flat color="blue" v-on:click="signIn()">Sign In</v-btn>
          </h5>
        </v-flex>
      </v-layout>
      <v-layout v-else row>
...
export default {
  ...
  methods: {
    ...
    scrollDown() {
      document.getElementById("scrollMsg").scrollTop = document.getElementById(
        "scrollMsg"
      ).scrollHeight;
    }
  }
